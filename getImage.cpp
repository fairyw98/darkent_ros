...
public:
    ...
    {
         image_sub_ = it_.subscribe("/usb_cam/image_raw", 1, &ImageConverter::convert_callback, this); //定义图象接受器，订阅话题是“camera/rgb/image_raw”
         image_pub_ = it_.advertise("/image", 1); //定义图象发布器
        //初始化输入输出窗口
    ...
    void convert_callback(const sensor_msgs::ImageConstPtr& msg) 
    {
        ...
        if(cv::waitKey(1)==32) image_pub_.publish(cv_ptr->toImageMsg());
    } 
...
{
    ros::init(argc, argv, "getImage"); //ros节点的初始化
    ImageConverter obj;
